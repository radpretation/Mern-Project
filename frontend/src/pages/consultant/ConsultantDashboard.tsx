...
export const ConsultantDashboard: React.FC = () => {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => toast.error(err.response?.data?.message || 'Failed to load client consultations.'))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Compliance Advisory Engagements</h2>
        <p className="text-sm text-slate-500 font-medium">
          Assigned advisory clients ready for evidence pre-audits and gap remediation guidance.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-base font-bold text-slate-800 flex items-center space-x-2">
          ...
          <span>Active Client Advisory Projects</span>
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {isLoading ? (
            <div className="col-span-full py-12 text-center text-slate-400">Loading advisory engagements...</div>
          ) : data?.complianceProjects?.length === 0 ? (
            <div className="col-span-full py-12 text-center text-slate-400">No client engagements currently assigned.</div>
          ) : (
            data?.complianceProjects?.map((cp: any) => (
              ...
              >
                ...
                <div className="mt-6 pt-3 border-t border-slate-100 flex justify-end">
                  <Link
                    to={`/consultant/audit-view?processId=${cp.processId?._id}&serviceId=${cp.serviceId}&customerId=${cp.customerId?._id}`}
                  ...
                  >
                    <span>Provide Consultation</span>
